...
public class RobotContainer {
    private static final double MAX_SPEED = TunerConstants.kSpeedAt12Volts.in(MetersPerSecond); // kSpeedAt12Volts desired top speed
    private static final double MAX_ANGULAR_RATE = RotationsPerSecond.of(0.0008).in(RadiansPerSecond); // 3/4 of a rotation per second max angular velocity
    ...
    private void configureBindings() {
        ...
        drivetrain.setDefaultCommand(
                drivetrain.applyRequest(() -> {
                    return drive.withVelocityX(driverJoystick.getLeftY() * MAX_SPEED)
                                ...
                                .withRotationalRate(driverJoystick.getRightX() * MAX_ANGULAR_RATE);
                })
        );
        ...
        operatorJoystick.L1().onTrue(new ElevatorCommand(ElevatorPosition.L1));
